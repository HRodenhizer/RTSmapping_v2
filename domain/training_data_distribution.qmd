...
train_meta = gcs_get_object(
  "gs://abrupt_thaw/RTS_MODEL_V2/DATA/TRAINING_DATA/metadata_region_hotfix.csv"
)

train_meta_old = gcs_get_object(
...
Why don't all the training tiles have a split group?

```{r}
train_meta_old = train_meta_old %>%
  left_join(splits_df, by = c("RegionName" = "ecoregion")) %>%
  st_as_sf(coords = c("centroid_lon", "centroid_lat"), crs = 4326) %>%
  st_transform(crs = 6931) %>%
  bind_cols(st_coordinates(.))

# train_points_disagreement = train_points |>
#   st_join(subregions) |>
#   mutate(incorrect_region = RegionName != ecoregion) |>
#   select(
#     Tile_ID,
#     TrainClass,
#     X,
...
ggplot(
  world_north |>
    st_crop(
      subregions |>
        filter(
          ecoregion %in% c("Beringia upland tundra", "Arctic foothills tundra")
        )
    )
) +
  geom_sf(
  ...
  geom_sf(
    data = train_points |>
      filter(RegionName == "Arctic foothills tundra"),
    aes(color = "RegionName == 'Beringia upland tundra'")
  )
```
