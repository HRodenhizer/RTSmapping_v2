...
train_meta <- read.csv(
  "./visualization/data/training_v1.0_metadata.csv"
...
splits <- yaml::read_yaml("./visualization/data/training_v1.0_splits.yaml")
splits_df <- map2_df(
  splits[1:3],
  names(splits)[1:3],
  ~ .x |>
...
train_hex <- train_points |>
  st_make_grid(
    cellsize = sqrt((100000 * 2) / (sqrt(3))) * 1000, # get short side length of hexagon from area == 100000 km^2, and convert to m
    square = FALSE
  ) |>
  st_as_sf() |>
  rename(geometry = x)
...
# st_write(
#   train_hex,
#   "./visualization/data/training_hex_grid.geojson",
# )
...
train_hex <- train_hex |>
  ...
  mutate(
    Buffer = (
      ...
      (
        sqrt((100000 * (Count / max(Count) * 0.98 + 0.02) * 2) / sqrt(3)) /
          2 *
...
```{r}
# st_write(train_hex, "./visualization/data/train_hex.geojson", delete_dsn = TRUE)
```
...
```{r}
# st_write(regions_hex, "./visualization/data/train_regions_hex.geojson", delete_dsn = TRUE)
```
